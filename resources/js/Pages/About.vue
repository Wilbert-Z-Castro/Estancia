...
const submit = () => {
    ...
    axios.post(route('egresados.EditaraMiPerfil', props.Egresado.idEgresado), form) // Enviar los datos con Axios
        .then(response => {
            // Éxito: Resetear formulario, cerrar Swal y mostrar mensaje de éxito
            form.reset();
            Swal.close();
            setTimeout(() => {
                Swal.fire({
                    ...
                    // Aquí puedes recargar la página si lo necesitas
                    window.location.reload(); // O redirigir con window.location.href si prefieres
                    // window.location.href = route('about'); // Redirige a otra página
                });
            }, 300);
        })
        .catch(error => {
